...
class FaceID:
    ...
    def __init__(self, cfg: Optional[FaceIDConfig] = None):
        ...
        # Schmitt-trigger state for authorization.
    ...
    def reset_auth(self) -> None:
        """Drop the latched authorization decision (new session or new template)."""
    ...
    def is_authorized(self, face_bgr: np.ndarray) -> bool:
        ...
        enter = float(self.cfg.cosine_thr)

        if not self.cfg.hysteresis:
            self._authorized = bool(s >= enter)
            return self._authorized

        release = min(float(self.cfg.release_thr), enter)
        ...
        if s >= enter:
            ...
            if self._below_streak >= int(self.cfg.release_frames):
                self._authorized = False
        else:
            # Between release and enter: hold whatever was decided last, and do
            # not let the ambiguous frame count toward a release.
            self._below_streak = 0
