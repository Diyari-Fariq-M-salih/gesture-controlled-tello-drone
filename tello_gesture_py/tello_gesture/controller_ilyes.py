...
"""Controller version with manual mode selection and face recognition for authorized user detection.
- NOTE: Works with the older class ModeManager instead of DeterministicModeManager. Tested successfully on February 9th, 2026.
"""
...
class Controller:
    def __init__(self, cfg: ControllerConfig, model_path: Optional[str] = None, labels_path: Optional[str] = None):
        ...
        # Multiple hands detection, then selection based on face recognition if enabled (if not, simply take the first index)
        self.hand = HandGesture(max_num_hands=2)  # Changed from 1 to 2 to detect two hands
